...
interface ProductListProps<T extends BaseProduct> {
  category: 'epoxicos' | 'abrazaderas' | 'kits'
  maxItems?: number
...
export function ProductList<T extends BaseProduct>({
  ...
  const [productos, setProductos] = useState<T[]>([])
    ...
    async function fetchProducts() {
      try {
        ...
        if (result.success && Array.isArray(result.data)) {
          setProductos(result.data.filter((p: T) => p.isActive !== false))
          console.log(`✅ Loaded ${result.data.length} ${category}`)
        } else {
  ...
  if (productos.length === 0) {
    return (
      <Alert>
        <Info className="h-4 w-4" />
        <AlertDescription>
          No se encontraron productos activos en la categoría de {category}.
        </AlertDescription>
      </Alert>
    )
  }

  const displayedProducts = productos.slice(0, maxItems)
  ...
  return (
    <>
          ...
          <Card key={producto._id} className="group hover:shadow-lg transition-shadow">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="line-clamp-2">
                {producto.description || producto.generic_type || `Producto especializado de ${category}`}
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-0">
              ...
              <Button asChild className="w-full">
                <Link href={`/productos/${category}/${producto._id}`}>
      ...
      {showMoreButton && productos.length > maxItems && (
        <div className="text-center">
          <Button variant="outline" size="lg">
            Ver Más Productos ({productos.length - maxItems} restantes)
          </Button>
        </div>
      )}
    </>
